...
void encoder_forward(float* out,
                   ...
                   int B, int T, int C) {
    for (int b = 0; b < B; b++) { // 遍历批中的每个样本
        for (int t = 0; t < T; t++) { // 遍历每个样本中的每个位置
...
void layernorm_forward(float* out, float* mean, float* rstd,
    ...
    for (int b = 0; b < B; b++) { // 遍历批次
        for (int t = 0; t < T; t++) { // 遍历token
...
void layernorm_backward(float* dinp, float* dweight, float* dbias,
                        ...
                        int B, int T, int C) {
    for (int b = 0; b < B; b++) { // 遍历批量数据
        for (int t = 0; t < T; t++) { // 遍历token长
            float* dout_bt = dout + b * T * C + t * C;
    ...
    for (int o = 0; o < OC; o++) {
        for (int b = 0; b < B; b++) {
            for (int t = 0; t < T; t++) {
                ...
                float d = dout_bt[o]; // 当前输出通道的梯度。
                if (dbias != NULL) { dbias[o] += d; } // 如果计算偏置梯度，则更新偏置梯度。
                for (int i = 0; i < C; i++) {
